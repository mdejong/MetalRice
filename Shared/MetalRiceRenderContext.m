...
#include "MetalRiceRenderContext.h"
...
@interface MetalRiceRenderContext ()
...
@implementation MetalRiceRenderContext
...
- (void) setupRenderPipelines:(MetalRenderContext*)mrc
{
  ...
  if (shader == nil) {
    ...
    shader = @"kernel_render_rice_undelta";
    
    self.computePipelineState = [mrc makePipeline:MTLPixelFormatR8Unorm
                                    pipelineLabel:@"RenderRice Pipeline"
    ...
    if (numThreadsInSIMDGroup != 32) {
      shader = @"kernel_render_rice_undelta_sync";
...
- (void) setupRenderTextures:(MetalRenderContext*)mrc
                ...
                cropFromSize:(CGSize)cropFromSize
                 ...
                 renderFrame:(MetalRiceRenderFrame*)renderFrame
{
             ...
             renderFrame:(MetalRiceRenderFrame*)renderFrame
{
  ...
  
  {
    ...
    if (debug) {
      NSLog(@"renderRice threadgroup %d x %d", (int)threadsPerThreadgroup.width, (int)threadsPerThreadgroup.height);
      NSLog(@"renderRice grid %d x %d", (int)threadgroupsPerGrid.width, (int)threadgroupsPerGrid.height);
    }
