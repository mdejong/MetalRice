...
#if defined(RICE_SYNC_THREADGROUPS)
    threadgroup_barrier(mem_flags::mem_threadgroup);
#endif // RICE_SYNC_THREADGROUPS
...
// Built on kernel_render_rice2_undelta except that the output
// is expanded to grayscale BGRA pixels in the compute shader.

kernel void kernel_render_rice_undelta(
                                       texture2d<half, access::write> outTexture [[ texture(0) ]],
                                       constant RiceRenderUniform & riceRenderUniform [[ buffer(0) ]],
                                       device uint32_t *inoutBlockOffsetTable [[ buffer(1) ]],
                                       const device uint32_t *inS32Bits [[ buffer(2) ]],
                                       ...
                                       )
{
  ...
  const ushort cropWidth = riceRenderUniform.cropWidth;
  const ushort cropHeight = riceRenderUniform.cropHeight;
  ...
  
  {
    ...
    // Write each Y but use a repeated read of the cached vlaue
    ...
    if (0)
    {
      ...
      thread uchar4 localCache[THREADGROUP_1D_DIM4];
      ...
      for ( ushort i = 0; i < 4; i++) {
        for ( ushort col = 0; col < THREADGROUP_1D_DIM4; col++ ) {
          ...
          uchar4 vec = localCache[col];
          
          half hVal = uint8_to_half(vec[i]);
          outPixelsVec[0] = hVal; // R
          outPixelsVec[1] = hVal; // G
          outPixelsVec[2] = hVal; // B
          outPixelsVec[3] = 1.0h; // A
    ...
    // At 4x, this seems just as fast as writing the Y values (slightly faster)
    ...
    if (1)
    {
      ...
      for ( ushort rw = 0; rw < 8; rw++ ) {
        ...
        const ushort2 bigBlockCoords = ushort2(col * 4, row);
        const ushort2 bigBlockRootCoordsM4 = bigBlockRootCoords * ushort2(4,1);
        ushort2 outCoords = bigBlockRootCoordsM4 + bigBlockCoords;
        
        half4 vecH;
        vecH[0] = uint8_to_half(vec[0]);
        vecH[1] = uint8_to_half(vec[1]);
        vecH[2] = uint8_to_half(vec[2]);
        vecH[3] = uint8_to_half(vec[3]);
        
        half4 outPixelsVec;
        outPixelsVec[3] = 1.0h; // A
        
        for ( ushort i = 0; i < 4; i++) {
          ushort2 outCoordsV = outCoords;
          outCoordsV.x += i;
          ...
          if (outCoordsV.x < cropWidth && outCoordsV.y < cropHeight) {
            outTexture.write(outPixelsVec, outCoordsV);
          }
        }
      }
    }
...
// Define a threadgroup sync safe version of this undelta method
...
kernel void kernel_render_rice2_undelta_sync(
                                        texture2d<half, access::write> outTexture [[ texture(0) ]],
                                        constant RiceRenderUniform & riceRenderUniform [[ buffer(0) ]],
                                        device uint32_t *inoutBlockOffsetTable [[ buffer(1) ]],
                                        const device uint32_t *inS32Bits [[ buffer(2) ]],
...
kernel void kernel_render_rice_undelta_sync(
                                             texture2d<half, access::write> outTexture [[ texture(0) ]],
                                             constant RiceRenderUniform & riceRenderUniform [[ buffer(0) ]],
                                             device uint32_t *inoutBlockOffsetTable [[ buffer(1) ]],
                                             const device uint32_t *inS32Bits [[ buffer(2) ]],
